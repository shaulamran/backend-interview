...
function extractTwoSubExpressions(expression) {
    ...
    while (i < expression.length) {
        if (expression[i] == '(') {
            parenthesisCounter++;
            if (stage == STAGES.FIRST_EXPRESSION_BEGINNING &&
        ...
        if (expression[i] == ')') {
            ...
            if (stage == STAGES.FIRST_EXPRESSION_END && parenthesisCounter == 1) {
                ...
                beginningOfSecondExpression = i + 2;
    ...
    const expressionOne = expression.substring(beginningOfFirstExpression, endOfFirstExpression);
    const expressionTwo = expression.substring(beginningOfSecondExpression, endOfSecondExpression);
...
exports.default = expressionToTree;
